...
const TicketDetailPage = () => {
  ...
  try {
    ...
    console.error("TicketDetailPage: Failed to parse user", e);
  }
      ...
      <main className="flex-1 w-full max-w-[1700px] mx-auto p-4 md:p-8 animate-in fade-in duration-500">
        <div className="rounded-2xl border bg-card shadow-xl overflow-hidden p-4 min-h-[720px]">
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">View ticket #{id}</p>
          </div>
          {/* Pass the ticketId through location state to TicketList */}
          <TicketList ticketIdFromRoute={id} />
        </div>
      </main>
    </div>
  );
